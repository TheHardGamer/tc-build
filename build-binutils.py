...
def invoke_configure(binutils_folder, build_folder, install_folder, target,
    ...
    if host_arch:
        configure += [
            f'CFLAGS=-O3 -march={host_arch} -mtune={host_arch} -fgraphite-identity -floop-nest-optimize',
            f'CXXFLAGS=-O3 -march={host_arch} -mtune={host_arch} -fgraphite-identity -floop-nest-optimize'
        ]
    else:
        configure += ['CFLAGS=-O3 -fgraphite-identity -floop-nest-optimize', 'CXXFLAGS=-O3 -fgraphite-identity -floop-nest-optimize']

    configure_arch_flags = {
        "arm-linux-gnueabi": [
            '--disable-multilib', '--disable-nls', '--with-gnu-as',
            '--with-gnu-ld' , '--enable-lto'
        ],
        "powerpc-linux-gnu":
